...
import { databaseUrlKey, dbSource, getSql } from "../db";
import {
  classifyAuthSecretSource,
  resolveDatabaseUrl,
  runtimeEnv,
...
export type AuthHealth = {
  ...
  pulse: Record<(typeof PULSE_TABLES)[number], boolean>;
  counts: {
    users: number;
    accounts: number;
    sessions: number;
...
> {
  ...
  return {
    ...
    secretSource: classifyAuthSecretSource(resolved.url),
    betterAuthHost: hostOnly(runtimeEnv("BETTER_AUTH_URL")),
    dbError: dbSource === "missing" ? "production_missing_url" : null,
    envFlags: {
      DATABASE_URL: Boolean(runtimeEnv("DATABASE_URL")),
      POSTGRES_URL: Boolean(runtimeEnv("POSTGRES_URL")),
      DATABASE_URL_UNPOOLED: Boolean(runtimeEnv("DATABASE_URL_UNPOOLED")),
      NEON_PROJECT_ID: Boolean(runtimeEnv("NEON_PROJECT_ID")),
...
const emptyCounts: AuthHealth["counts"] = {
  users: 0,
  accounts: 0,
  sessions: 0,
...
function emptyPulse(names: Set<string> = new Set()): AuthHealth["pulse"] {
  return {
    profiles: names.has("profiles"),
    exercises: names.has("exercises"),
    routines: names.has("routines"),
    routine_exercises: names.has("routine_exercises"),
    workouts: names.has("workouts"),
    workout_sets: names.has("workout_sets"),
  };
}

/**
 * Aggregate, no-PII snapshot of the auth tables on the SAME SQL client the
 * app uses (`getSql()` → Neon when a Postgres URL is set).
 */
export async function getAuthHealth(): Promise<AuthHealth> {
  ...
  try {
    ...
    const tables = await sql.query<{ table_name: string }>(
      `select table_name from information_schema.tables
       where table_schema = 'public' and table_name in (
         'user', 'account', 'session', '_migrations',
    ...
    return {
      ...base,
      dbError: base.dbError ?? "unavailable",
      schema: {
        hasUserTable: false,
        hasAccountTable: false,
        hasPasswordColumn: false,
