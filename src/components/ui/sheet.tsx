...
export function SheetContent({
  ...
  return (
    <DialogPrimitive.Portal>
      <DialogPrimitive.Overlay className="pulse-overlay fixed inset-0 z-50 bg-black/50 backdrop-blur-[6px]" />
      <DialogPrimitive.Content
        className={cn(
          "pulse-sheet fixed z-50 flex flex-col bg-card shadow-float hairline overscroll-contain",
          side === "bottom" &&
            "inset-x-0 bottom-0 max-h-[92dvh] overflow-hidden rounded-t-3xl pb-[max(1rem,env(safe-area-inset-bottom))]",
          side === "right" &&
            "pulse-sheet-right inset-y-0 right-0 h-full w-[min(100%,420px)] max-w-full overflow-hidden rounded-l-3xl",
        )}
        {...props}
      >
        {side === "bottom" && (
        ...
        <div className={cn("min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain", className)}>
          {children}
        </div>
