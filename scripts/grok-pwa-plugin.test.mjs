...
function emptyOgRoot() {
  return mkdtempSync(join(tmpdir(), "grok-og-iso-"));
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: emptyOgRoot(), site: {} });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: emptyOgRoot(), site: {} });
  const chunks = [
...
test("site title Pulse names the manifest Pulse with Pulse icons", () => {
  const manifest = JSON.parse(
    renderWebManifest("pulse-psi-blond.vercel.app", {
      site: { title: "Pulse", description: "Tu entrenamiento. Tu progreso. Tu ritmo." },
    }),
  );
  assert.equal(manifest.name, "Pulse");
  assert.equal(manifest.short_name, "Pulse");
  assert.equal(manifest.display, "standalone");
  assert.equal(manifest.start_url, "/");
  assert.equal(manifest.scope, "/");
  assert.equal(manifest.description, "Tu entrenamiento. Tu progreso. Tu ritmo.");
  assert.ok(manifest.icons.some((icon) => icon.src === "/icons/pulse-192.png"));
  assert.ok(manifest.icons.some((icon) => icon.src === "/icons/pulse-512.png"));
  const raw = JSON.stringify(manifest);
  assert.equal(raw.includes("Grok App"), false);
  assert.equal(raw.includes("/__grok/icon"), false);
});

test("does not inject grok apple-touch-icon when the app already provided one", () => {
  const out = injectGrokPwaHead(
    '<html><head><link rel="apple-touch-icon" href="/icons/pulse-180.png"></head></html>',
  );
  assert.match(out, /href="\/icons\/pulse-180\.png"/);
